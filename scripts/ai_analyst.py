"""
AI_analyst.py
Sends raw OHLCV price data to AI AI-opus-4-6 and asks it to perform
...
def analyse_with_AI(
    ...
    """
    Send price data to AI for full ICT analysis.
...
"""
    ...
    for block in response.content:
        print(f"    [AI] Block type: {block.type}")
        if block.type == "text":
            raw_text += block.text
    ...
    if "<result>" not in raw_text or "</result>" not in raw_text:
        print(f"    [AI] No <result> tags found. Full response:\n{raw_text}")
        raise ValueError("AI response missing <result> tags")

    json_str = raw_text.split("<result>")[1].split("</result>")[0].strip()
    print(f"    [AI] Extracted JSON ({len(json_str)} chars)")

    try:
        result = json.loads(json_str)
    except json.JSONDecodeError as e:
        print(f"    [AI] JSON parse failed: {e}")
        print(f"    [AI] JSON string:\n{json_str}")
        raise

    # Always inject current_price and symbol (not in AI's output schema)
